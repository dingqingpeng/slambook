...
int main(int argc, char const *argv[])
{
    ...
    for(int index = 0; index < 600; index++)
    {
        ...
        for( auto m: measurements )
        {
            ...
            cv::circle ( img_show, cv::Point2d ( pixel_prev ( 0,0 ), pixel_prev ( 1,0 ) ), 8, cv::Scalar ( 0,0,255 ), 2 );
            cv::circle ( img_show, cv::Point2d ( pixel_now ( 0,0 ), pixel_now ( 1,0 ) +color.rows ), 8, cv::Scalar ( 0,0,255 ), 2 );
