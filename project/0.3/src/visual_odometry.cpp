...
VisualOdometry::VisualOdometry()
    : state_( INITIALIZING ), map_( new Map ), ref_( nullptr ), cur_( nullptr ), 
...
void VisualOdometry::featureMatching()
{
    ...
    matcher_flann_.match( descriptors_in_ref_, descriptors_in_cur_, matches );
    ...
    /* Camera trajectory seems to be very sensitive to number or matches.
       While selecting good matches, if use "<=" instead of "<", camera trajectory would be very different,
       a transform with very large motion would even occure. 
       The code below outputs difference in numbers of two types of operator.*/
       
    /* vector<cv::DMatch>   feature_matches_1_;
    feature_matches_1_.clear();
    for(cv::DMatch& m: matches)
    {
        if (m.distance <= max<float>(match_ratio_*min_dist, 30.0))
        {
            feature_matches_1_.push_back(m);
        }
    }
    cout<<"good matches: "<<feature_matches_.size() << "  |  " << 
    feature_matches_1_.size() << "  |  " << feature_matches_1_.size() - feature_matches_.size() <<endl; */
...
void VisualOdometry::poseEstimationPnP()
{
    ...
    // Use bundle adjustment to optimize pose 
    ...
    pose->setEstimate ( g2o::SE3Quat (
        Tcr_estimated_.rotation_matrix(), 
        Tcr_estimated_.translation()
    ) );
...
bool VisualOdometry::checkEstimatedPose()
{
    ...
    if ( d.norm() > 5.0 )
    {
        ...
        // cout << Tcr_estimated_.translation() << endl;
        cout << (Tcr_estimated_ * ref_->T_c_w_).translation() << endl;
        cv::waitKey();
        return false;
    }
    return true;
